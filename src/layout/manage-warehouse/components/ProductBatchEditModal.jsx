// src/layout/manage-warehouse/components/ProductBatchEditModal.jsx
import { Modal, Form, Input, InputNumber, Select, Button, message, DatePicker } from 'antd';
import { useState, useEffect } from 'react';
import { updateProductBatch } from '../../../services/productBatchService';
...
function ProductBatchEditModal({ open, onClose, onSuccess, batch }) {
        ...
        if (open) {
            ...
            if (batch) {
                form.setFieldsValue({
                    ...
                    unit: batch.unit,
                    note: batch.note,
                    receivedAt: batch.receivedAt ? dayjs(batch.receivedAt) : null,
                    expiredAt: batch.expiredAt ? dayjs(batch.expiredAt) : null,
                    providerId: batch.providerId,
                    subSubcategoryId: batch.subSubcategoryId,
    ...
    // Update selected category when subSubcategories load or batch changes
    useEffect(() => {
        if (batch?.subSubcategoryId && subSubcategories.length > 0) {
            const category = subSubcategories.find(cat => cat.subSubcategoryId === batch.subSubcategoryId);
            setSelectedCategory(category);
        }
    }, [batch, subSubcategories]);
    ...
    const handleFinish = async (values) => {
        ...
        try {
            ...
            const response = await updateProductBatch(batch.batchId, payload);

            if (response.data.type === 'GOOD') {
                message.success('Cập nhật product batch thành công!');
                form.resetFields();
                onSuccess?.();
                onClose();
            } else {
                message.error(response.data.message || 'Cập nhật product batch thất bại!');
            }
        } catch (error) {
    ...
    return (
        <Modal
            title={`Chỉnh sửa Product Batch #${batch?.batchId}`}
            open={open}
...
export default ProductBatchEditModal;
